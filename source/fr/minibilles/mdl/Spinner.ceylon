...
shared class Spinner (
    ...
    id, spinnerClazz(clazz, active, singleColor),
...
Attribute<String> spinnerClazz(Attribute<String> clazz, Boolean active, Boolean singleColor) {
    variable [String+] toAdd = ["mdl-spinner", "mdl-js-spinner"];
    if (active) { toAdd = toAdd.withTrailing("is-active"); }
    if (singleColor) { toAdd = toAdd.withTrailing("mdl-spinner--single-color"); }
