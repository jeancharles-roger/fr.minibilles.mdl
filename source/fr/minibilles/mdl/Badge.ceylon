...
shared class Badge(
    ...
    shared Boolean overlap = false,
    "Applies open-circle effect to badge"
    shared Boolean noBackground = false,
    ...
    id,badgeClazz(clazz, overlap, noBackground),
    accessKey, contentEditable,contextMenu,
    dir, draggable, dropZone,
    hidden, lang, spellcheck,style,
    tabIndex, title, translate,
    attributes.withTrailing("data-badge" -> content), children
) {

}

Attribute<String> badgeClazz(Attribute<String> clazz, Boolean overlap, Boolean noBackground) {
    variable [String+] toAdd = ["mdl-badge"];
    if (overlap) { toAdd = toAdd.withTrailing("mdl-badge--overlap"); }
    if (noBackground) { toAdd = toAdd.withTrailing("mdl-badge--no-background"); }
