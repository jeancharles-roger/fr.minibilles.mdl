...
shared class DialogButton(
    shared Boolean close = false,
    shared {Content<PhrasingCategory>*} contents = []
) {
...
shared class Dialog(
    ...
    shared {Content<PhrasingCategory>*} dialogTitle,

    "Actions for the dialog"
    shared {DialogButton*} actions,
    ...
    shared {Content<FlowCategory>*} contents = [])
    extends Element(
    "dialog", id, dialogClazz(clazz), accessKey, contentEditable,
    contextMenu, dir, draggable, dropZone, hidden, lang,
    spellcheck, style, tabIndex, title, translate,
    attributes, dialogChildren(dialogTitle, contents, actions)
)
    satisfies FlowCategory {
}

Attribute<String> dialogClazz(Attribute<String> clazz) {
    return appendClazz(clazz, ["mdl-dialog"]);
}

{Content<FlowCategory>*} dialogChildren(
    {Content<PhrasingCategory>*} title, {Content<FlowCategory>*} contents,
    {DialogButton*} actions
) =>
    {
        H4 { clazz = "mdl-dialog__title"; *title },
        Div { clazz = "mdl-dialog__content"; *contents },
        Div {
            clazz = "mdl-dialog__actions";
            *actions.map((b) => HtmlButton {
                type = ButtonType.button;
                clazz = if (b.close) then "mdl-button close" else "mdl-button";
                *b.contents
            })
        }
    };
