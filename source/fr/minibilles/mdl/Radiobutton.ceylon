...
shared class RadioGroup(shared String name) {
    shared actual String string = "RadioGroup ``name``";
}

shared class RadioButton (

    "Group for this radio button"
    shared RadioGroup group,

    "Id tag for the input"
    shared String inputId,

    "Value for the option"
    shared String val,

    "Starting status of the radio button"
    shared Boolean checked = false,
    ...
    "Attribute represents the form element that the label element is associated with (its form owner). If specified, the value of the attribute must be the ID of a form element in the same document."
    ...
    id, radioButtonClazz(clazz, rippleEffect),
    accessKey, contentEditable, contextMenu, dir, draggable, dropZone,
    hidden, lang, spellcheck, style, tabIndex, title, translate, inputId,
    form,  attributes, radioButtonChildren(children, inputId, val, checked, group)
) {

}

Attribute<String> radioButtonClazz(
        Attribute<String> clazz, Boolean rippleEffect
) {
    variable [String+] toAdd = ["mdl-radio", "mdl-js-radio"];
    if (rippleEffect) { toAdd = toAdd.withTrailing("mdl-js-ripple-effect"); }
    return appendClazz(clazz, toAdd);
}

{Content<PhrasingCategory>*} radioButtonChildren({Content<PhrasingCategory>*} children, String inputId, String val, Boolean checked, RadioGroup group) => {
    Input { type = InputType.radio; id = inputId; clazz = "mdl-radio__button"; name=group.name; val = val; checked = checked;},
    Span { clazz="mdl-radio__label"; *children}
};
