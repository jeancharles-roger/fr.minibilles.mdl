...
shared class Switch (
    ...
    "Id tag for the input inside switch"
    shared String inputId,

    "Starting status of the switch"
    ...
    id, switchClazz(clazz, rippleEffect),
    ...
    form,  attributes, switchChildren( inputId, checked)
) {

}

Attribute<String> switchClazz(
        Attribute<String> clazz, Boolean rippleEffect
) {
    variable [String+] toAdd = ["mdl-switch", "mdl-js-switch"];
    if (rippleEffect) { toAdd = toAdd.withTrailing("mdl-js-ripple-effect"); }
    return appendClazz(clazz, toAdd);
}

{Content<PhrasingCategory>*} switchChildren(String inputId, Boolean checked) => {
    Input { type = InputType.checkbox; id = inputId; clazz = "mdl-switch__input"; checked = checked; },
    Span { clazz="mdl-switch__label"; }
};
