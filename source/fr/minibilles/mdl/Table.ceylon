...
shared class Table(
    ...
    shared Boolean selectable = false,
    ...
    {Content<Caption|ColGroup|THead|TBody|TFoot|Tr>*} children = []
) extends HtmlTable(
    id, tableClazz(clazz, selectable), accessKey, contentEditable, contextMenu,
    dir, draggable, dropZone, hidden, lang, spellcheck,
    style, tabIndex, title, translate, attributes, children
)
    satisfies FlowCategory {
}


Attribute<String> tableClazz(Attribute<String> clazz, Boolean selectable) {
    variable [String+] toAdd = ["mdl-data-table", "mdl-js-data-table"];
    if (selectable) { toAdd = toAdd.withTrailing("mdl-data-table--selectable"); }
