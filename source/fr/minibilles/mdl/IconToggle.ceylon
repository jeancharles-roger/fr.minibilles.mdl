...
shared class IconToggle (

    "Icon name"
    shared String name,

    "Id tag for the input inside text field"
    shared String inputId,

    "Starting status of the toggle"
    ...
    id, iconToggleClazz(clazz, rippleEffect),
    ...
    form,  attributes, iconToggleChildren(name, inputId, checked)
) {

}

Attribute<String> iconToggleClazz(
        Attribute<String> clazz, Boolean rippleEffect
) {
    variable [String+] toAdd = ["mdl-icon-toggle", "mdl-js-icon-toggle"];
    if (rippleEffect) { toAdd = toAdd.withTrailing("mdl-js-ripple-effect"); }
    return appendClazz(clazz, toAdd);
}

{Content<PhrasingCategory>*} iconToggleChildren(String name, String inputId, Boolean checked) => {
    Input { type = InputType.checkbox; id = inputId; clazz = "mdl-icon-toggle__input"; checked = checked; },
    Span { clazz="mdl-icon-toggle__label material-icons"; name}
};
