...
shared class Checkbox (
    ...
    id, checkboxClazz(clazz, rippleEffect),
    ...
    form,  attributes, checkboxChildren(children, inputId)
) {

}

Attribute<String> checkboxClazz(
        Attribute<String> clazz, Boolean rippleEffect
) {
    variable [String+] toAdd = ["mdl-checkbox", "mdl-js-checkbox"];
    if (rippleEffect) { toAdd = toAdd.withTrailing("mdl-js-ripple-effect"); }
    return appendClazz(clazz, toAdd);
}

{Content<PhrasingCategory>*} checkboxChildren({Content<PhrasingCategory>*} children, String inputId) => {
    Input { type = InputType.checkbox; id = inputId; clazz = "mdl-checkbox__input"; },
    Span { clazz="mdl-checkbox__label"; *children}
};
